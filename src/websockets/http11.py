...
class Request(NamedTuple):
    ...
    @classmethod
    def parse(
        cls, readline: Callable[[], Generator[None, None, bytes]]
        ...
        :param readline: generator-based coroutine that reads a LF-terminated
            line or raises an exception if the end of file is reached
        ...
        try:
            request_line = yield from parse_line(readline)
        ...
        headers = yield from parse_headers(readline)
...
class Response(NamedTuple):
    ...
    def parse(
        ...
        try:
            status_line = yield from parse_line(readline)
        except EOFError as exc:
        ...
        return cls(status_code, reason, headers)
...
def parse_headers(
    readline: Callable[[], Generator[None, None, bytes]]
    ...
    for _ in range(MAX_HEADERS + 1):
        try:
            line = yield from parse_line(readline)
        except EOFError as exc:
